...
void Player::initializeHand(Hand* h1)
...
{
	bool t = true;
	cout << "Please select the country's number you want to move troops to :" << endl;
	do {
		int i = 0;
...
void Player::placingArmy(int &rewardedArmy)
{
	...
	while (rewardedArmy != 0)
	{
		...
		do{
		cout << "Please select the country's number you want to reinfore:" << endl;
		for (vector<Country>::iterator it = countries->begin(); it != countries->end(); ++it) {
